import { attributes, withSpan, withSpanSync } from "./telemetry.js";
...
export async function resolveEveEntities(
  ...
  return withSpan(
    "eve.resolveEveEntities",
    {
      "eve.input.mode": "names" in input ? "names" : "ids",
      "eve.input.count":
        "names" in input ? input.names.length : input.ids.length,
    },
    async () => {
      if ("names" in input) {
        const response = await client.call({
          operationId: "PostUniverseIds",
          body: withSpanSync("eve.entities.deduplicate_names", () =>
            uniqueNames(input.names),
          ),
        });
        ...
        attributes({ "eve.output.candidate_count": candidates.length });
        return {
          matchMode: "exact",
