...
export function renderSkillPlanGuidance(request: SkillPlanRequest): string {
  ...
  return [
    ...
    "Call generate_skill_plan with characterId, the verified target(s), and the selected queuePolicy. It retrieves scoped character skills and queue, removes completed permanent levels, accounts for future queue commitments, expands all six SDE prerequisite slots, deduplicates shared skill-level nodes, topologically sorts and independently replays the order. It returns compact ordered plan rows in data, counts, source evidence and estimated remaining SP. Request response.path=[resolvedTargets], [graph], [retainedQueue], [acquisitionChecks] or [trainingText] independently; retain output.snapshot across views and follow output.nextOffset/omitted until the requested evidence is complete. Do not reproduce any of these calculations in prose, mental maps, ad hoc scripts or a second inferred plan.",
    "On needs_target_selection, read data and follow output to retrieve all target/candidate slices, then resolve the choices and retry. On failed authentication use the normal SSO flow or authorize_eve_character; never ask for tokens or commands. Missing, malformed or incomplete character data is not an untrained character. On stale completed-queue/skills conflicts, refresh the evidence and retry; never promote a level just because a timestamp is in the past. If the planner cannot return a complete result, do not emit an importable list. get_skill_dependencies can still show public requirements explicitly labelled as not personalized.",
    "Inspect staticData.buildNumber, source URL, freshness/stale warnings, characterSources, baseline, counts, dependencyChecked, data, output and caveats. Retrieve retainedQueue separately when needed; omitted data is not empty evidence. Do not merge graphs from different builds. Trained levels are permanent progress; active levels describe current usability; queued levels are conditional future progress. Inactive trained levels should be flagged, not retrained. Injected level-0 skills do not automatically need another book. Preserved-queue trainingText must be labelled additions after that queue, not a standalone plan. Reorder text includes existing commitments, not only the requested goal.",
