...
export function registerCartography(
  ...
  server.registerTool(
    "render_eve_map",
    {
      ...
      description: `Render an EVE Online server-computed route by routeId, or a context map from an explicit boundary and pointsOfInterest. Nonempty raw routes are rejected. Never creates plans, chooses destinations, calculates routes or recommends activities; call plan_eve_route first. Crowded route maps retry on a larger padded canvas. If no readable map fits, return a clear error and the stored route as text; no itinerary images are generated. Never redraw, trim or reconstruct a failed map. Request boundary:{kind:'neighborhood',center:<exact name or ID>,jumps:1} directly for immediate permanent-stargate neighbors, without ESI discovery or per-neighbor calls. Only jumps:1 is supported; context boundaries are limited to 250 systems. Writes private expiring SVG artifacts and optionally a PNG preview. No EVE login or game-state changes. ${PLANNING_AUTHORITY}`,
